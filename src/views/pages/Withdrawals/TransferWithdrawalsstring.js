...
function TransferWithdrawalsstring() {
  let filterData = {};
  ...
  const fetchTransferWithdrawals = async (currentPage) => {
    try {
      ...
      const response = await GettransferredWithdrawalsApi(
        ...
        Symbol,
        controller,
        // Passing filterData to the API call
      );
...
const handleDownload = async () => {
  try {
   ...
   const Symbol = "TON";
    const fullLimit = totalRecord; // make sure this is a number

    const response = await GettransferredWithdrawalsApi(
      filtersData?.search,
      startDate,
      endDate,
      1,              // currentPage fixed to 1 for export
      filterData,
      fullLimit,
      Symbol,
      controller
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Transferred Withdrawals ");
    XLSX.writeFile(workbook, "TransferredWithdrawals.xlsx");
  ...
  return (
    <Box className={classes.main}>
      <Box className="displaySpacebetween">
        <GoBack title={"Transferred Withdrawals String TON"} />
      ...
      <Box mt={3} mb={3}>
        <Paper elevation={3}>
          <Filtter
            ...
            excelTableName="Transferred Withdrawals String TON"
            apiEndPoint="Pending"
          />
            ...
            <Table>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell>Trasferred ON</TableCell>
...
export default TransferWithdrawalsstring;
