...
function Approvedwithdrawalspinsol() {
  let filterData = {};
...
console.log("Is Private Key Input Visible:", isPrivateKeyInputVisible);
  ...
  const settings = async () => {
    try {
      ...
      if (response && response.data && Array.isArray(response.data.withdrawMethods)) {
        console.log("Withdraw Methods Response:", response.data.withdrawMethods); // Log the withdrawMethods data

        const filteredData = response.data.withdrawMethods.filter(
          (item) => item.status === 'ACTIVE' && item.Network === 'SOL'
        );

        console.log("Filtered Data:", filteredData); // Log the filtered data after applying filter

        if (filteredData.length > 0) {
          console.log(filteredData[0].Token_Mint, 'filteredData[0].Token_Mint');
          setTokenMint(filteredData[0].Token_Mint); // Set the tokenMint state with the filteredData[0].Token_Mint
        } else {
          console.log('No active SOLana network found');
        }
...
console.log("Selected Rows after Select All:", selectedRows);
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, "Approved Withdrawals Spin SOL");
      XLSX.writeFile(workbook, "Approved_Withdrawals_Spin_SOL.xlsx");
    } catch (error) {
    ...
    console.log(privateKey, "privateKey");
      ...
      if (tokenmint == "So11111111111111111111111111111111111111112") {
        ...
        for (const row of selectedRows) {
          ...
          console.log(row.user.wallet_address, feeWallet, 'transactionAmount, feeAmount');


          const transferInstruction = SystemProgram.transfer({
            fromPubkey: sender.publicKey,
            toPubkey: new PublicKey(row.user.wallet_address),
        ...
        for (const some of selectedRows) {
          ...
          try {
            const receiverPublicKey = new PublicKey(some.user.wallet_address);
            ...
            const toUserTokenAccount = await getOrCreateAssociatedTokenAccount(
              ...
              console.error(`Error creating toTokenAccount for user ${some.user.wallet_address}:`, error);
              return null;
            });
            ...
            console.error(`Error processing row ${some.user.wallet_address}:`, error);
          }
        }
  ...
  return (
    <Box className={classes.main}>
      <Box className="displaySpacebetween">
        <GoBack title={"Approved Withdrawals Spin Sol"} />
...
export default Approvedwithdrawalspinsol;
