...
function Approvedwithdrawaltetris() {
  let filterData = {};
  ...
  const settings = async () => {
    try {
      // Read tokendata fresh each time, inside the function
      const tokendata = window.sessionStorage.getItem("tetris");
      console.log("Token Data in settings():", tokendata);

      if (!tokendata) {
        console.warn("No tetris token found in sessionStorage. Skipping settings fetch.");
        return;
      }
      ...
      console.log("Withdraw Limits Response:", response);

      // Handle 403 or other auth errors gracefully
      if (response && response.status === 403) {
        console.warn("Forbidden (403): You do not have permission to access this resource. This may be a token or auth issue.");
        // Don't throw — allow page to continue loading without token mint
        return;
      }

      if (response && response.data && response.data.data) {
        console.log("Withdraw Methods Response:", response.data.data);

        const filteredData = response.data.data;
        console.log("Filtered Data:", filteredData);

        setTokenMint(filteredData.token_Mint);
      } else {
  ...
  useEffect(() => {
    // Only call settings if we have a token in sessionStorage
    const tokendata = window.sessionStorage.getItem("tetris");
    if (tokendata) {
      settings();
    }
  ...
  //     console.log("API Response getallwithdrawstatusdrive", response);
  ...
  const handleApproveClick = async () => {
    ...
    const tokendata = window.sessionStorage.getItem("tetris");
    console.log("carrace from sessionStorage handleApproveClick:", tokendata);
...
console.log("selectedRows",selectedRows);
        ...
        for (const row of selectedRows) {
          ...
          console.log(row.user.walletAddress, feeWallet, 'transactionAmount, feeAmount');
        ...
        const speexecuteTransaction = async (batchInstructions) => {
          try {
            ...
            console.log("speexecuteTransaction",transaction);
            ...
            try {
              ...
              // Accept either HTTP 200 or API-style responseCode === 200
              const successFromHttp = response && response.status === 200;
              const successFromApi = response && response.data && (response.data.responseCode === 200 || response.data.status === 200);
              
              if (successFromHttp || successFromApi) {
                const successMsg = response?.data?.responseMessage || response?.data?.message || "Transaction data successfully posted.";
                toast.success(successMsg);
                setTimeout(() => {
                  window.location.reload();
                }, 1000);
              } else {
                // Log full response for debugging and show any server message
                console.error("POST transferWithdrawTetris failed:", response && (response.data || response));
                const serverMsg = response?.data?.message || response?.data?.responseMessage || response?.data?.error || response?.statusText || "Failed to post transaction data.";
                toast.error(serverMsg);
              }
            } catch (error) {
              console.error("Error posting transaction data:", error);
              toast.error(error?.response?.data?.message || error?.message || "An error occurred while posting transaction data.");
            } finally {
              setIsGameUpdating(false);
            }
            // } else {
        ...
        for (const some of selectedRows) {
          ...
          try {
            ...
            console.log(fromTokenAccount.address,
              ...
              sender.publicKey,
              userAmount,'userAmount, feeAmount');
  ...
  useEffect(() => {
    ...
    if (tokendata) {
      const source = axios.CancelToken.source();
      console.log("Page mounted or filters/page changed — calling gameManagementApi...");
      gameManagementApi(source);
      ...
      console.warn("No token found in sessionStorage for 'carrace'. API not called.");
    }
  }, [page, filtersData]);
      ...
      <Box mt={3} mb={3}>
        <Paper elevation={3}>
          <Filtter
            ...
            excelTableName="Approved Withdrawals Tetris Sol"
            apiEndPoint="Pending"
          />
...
export default Approvedwithdrawaltetris;
